...
import testetFakerFactory from "./testetFakerFactory";
...
const TestetFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = testetFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("testet").create(elem));
...
export default connect(mapState, mapDispatch)(TestetFakerDialogComponent);
