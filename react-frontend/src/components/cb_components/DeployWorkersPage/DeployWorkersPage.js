...
const DeployWorkersPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          deployerJobId: urlParams.singleDeployJobsId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("deployWorkers").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="mt-5">
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <DeployWorkersEditDialogComponent
